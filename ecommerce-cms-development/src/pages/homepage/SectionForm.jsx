...
const SectionForm = ({
  ...
  const [selectedProducts, setSelectedProducts] = useState(
    section.config?.selected_product_ids
      ? section.config.selected_product_ids.map((id) => {
          ...
          const resolved = section.config?.selected_products?.find?.(
            (p) => p.id === id,
          );
          return resolved
            ? {
                id: resolved.id,
                name:
                  resolved.product_translations?.[0]?.title ||
                  resolved.title ||
                  `Product #${resolved.id}`,
              }
            : { id, name: `Product #${id}` };
  ...
  const handleProductSearchChange = (e) => {
    ...
    productSearchTimeout.current = setTimeout(async () => {
      try {
        const data = await ProductServices.getProductSuggestions({
          search: value.trim(),
          sku: value.trim(),
          limit: 10,
        });
  ...
  const handleAddProduct = (product) => {
    ...
    setSelectedProducts((prev) => {
      ...
      const next = [...prev, { id: product.id, name: title, sku: product.sku }];
      onUpdate({
        ...section,
        config: {
          ...section.config,
            ...
            {section.config?.category_id === "mixed" && (
              <div className="bg-white p-4 rounded-lg border-2 border-gray-200">
                ...
                <p className="text-xs text-gray-500 mb-3">
                  Optional: Select specific categories to pool products from.
            ...
            {section.config?.category_id === "mixed" && (
              <div className="bg-white p-4 rounded-lg border-2 border-orange-200">
                <label className="flex items-center gap-2 text-sm font-semibold text-gray-700 mb-1">
                  <Package size={16} className="text-orange-600" /> Selected
                  Products
                ...
                <p className="text-xs text-gray-500 mb-3">
                  Optional: Manually pin specific products. When set, category
                  pool is ignored. Search by product name, ID, or SKU.
                </p>
                      ...
                      {productSearchResults.map((product) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {alreadyAdded && (
                              <span className="ml-auto shrink-0 text-xs text-green-600 font-semibold">
                                Added
                              </span>
                            )}
                          </button>
                ...
                {selectedProducts.length > 0 ? (
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {selectedProducts.map((product, idx) => (
                        <div
                          key={product.id}
                          className="flex items-center gap-3 px-3 py-2 w-fit bg-orange-50 border border-orange-200 rounded-lg"
                        >
                          {/* <span className="text-xs text-orange-400 font-mono w-5 shrink-0">
													#{idx + 1}
												</span> */}
                  ...
                  <div className="text-center py-6 text-gray-400 border-2 border-dashed border-gray-200 rounded-lg">
                    ...
                    <p className="text-xs">
                      No products selected. Category pool will be used.
                    </p>
                  </div>
                )}
              </div>
