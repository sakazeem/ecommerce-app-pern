        ...
        if (isAuthenticated) {
          ...
          try {
            await CartService.addToCart({
              ...
              quantity: Math.min(quantity, maxStock), // ✅ limit here
            });
          ...
          if (existing) {
            updatedCart = cart.map((item) => {
              const isSame = item.selectedVariant
                ? item.id === product.id &&
                  item.selectedVariant.id === product.selectedVariant?.id
                : item.sku === product.sku;
              ...
              return {
                ...item,
                quantity: Math.min(newQty, maxStock), // ✅ clamp
              };
            });
          } else {
            ...
            updatedCart = [
              ...
              {
                ...product,
                quantity: Math.min(quantity, maxStock), // ✅ clamp
              },
            ];
          }
        ...
        if (isAuthenticated && product.cartItemId) {
          try {
            ...
            await CartService.updateCartItem(
              product.cartItemId,
              Math.min(quantity, maxStock), // ✅ clamp before API
            );
