...
export const useCartStore = create(
  persist(
    (set, get) => ({
      cart: [],
      favourites: [],
      _syncing: false, // guard: prevents verifyAndSyncCart racing with mutations
        ...
        if (isAuthenticated) {
          try {
            ...
            const normalized = (res.cart || []).map((item) => {
              ...
              // Junction data is at branch.pvb (through: { as: 'pvb' } in cartIncludes)
              const pvb = v?.branches?.[0]?.pvb;
              const selectedVariant = v
                ? {
                    id: v.id,
                    sku: v.sku,
              ...
              return {
                ...
                sku: v?.sku || item.product?.sku,
                base_price: item.product?.base_price,
                base_discount_percentage:
                  item.product?.base_discount_percentage,
      ...
      verifyAndSyncCart: async (isAuthenticated) => {
        const { cart, _syncing } = get();
        if (!cart.length) return;
        // Authenticated users get fresh data via loadCart (DB is source of truth).
        // Skip verify to avoid overwriting optimistic state mid-mutation.
        if (isAuthenticated) return;
        // Prevent concurrent verify calls from racing with add/remove/update
        if (_syncing) return;
        set({ _syncing: true });
