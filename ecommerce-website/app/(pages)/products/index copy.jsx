...
import SpinLoaderProducts from "@/app/components/Shared/SpinLoaderProducts";
import Footer from "@/app/components/Themes/KidsTheme/Footer";
	...
	return (
		<>
			<main className="max-md:hidden">
				<section className="container-layout section-layout-top">
			...
			<main className="md:hidden">
				<section className="section-layout-top">
					{/* Mobile Filter Button */}
					<div className="container-layout md:hidden flex justify-between items-center mb-4">
						<button
							onClick={() => setMobileFilterOpen(true)}
							className="flex items-center gap-2 px-4 py-2 border rounded-lg bg-white shadow-sm">
							<SlidersHorizontal size={18} />
					...
					<section className="grid md:grid-cols-4 gap-10 relative">
						...
						<section className="md:col-span-3">
							<h4 className="container-layout h4 font-bold mb-4 border-b pb-1">
								Results
								{/* {data?.pages[0]?.total > 0 && (
								<span className="text-muted ml-2 text-sm font-normal">
									(Showing {products.length} of {data.pages[0].total})
							...
							{initialLoader || isLoading ? (
								...
								<>
									...
									{/* End of results indicator */}
									{!hasNextPage && products.length > PRODUCTS_PER_PAGE && (
										<p className="container-layout text-center text-muted py-6 p4">
											You've reached the end of the results
										</p>
									)}
								...
								<>
									<h4 className="container-layout h4 text-muted text-center section-layout">
										No products found, please adjust filters
									</h4>
									<div className="md:hidden">
										<Footer />
