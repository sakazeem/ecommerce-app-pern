...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    trackEvent("InitiateCheckout", {
      ...
      contents: cart.map((item) => ({
        ...
        item_price: Number(item.selectedVariant?.price || 0),
      })),
  ...
  const handleSubmit = async (e) => {
    ...
    await logActivity({
      ...
      payload: { hasReceipt: !!ibftReceipt },
    });

    await OrderService.confirmOrder(fd)
      .then((res) => {
        // ✅ Success UI state
        const orderId = res?.order?.tracking_id;
        ...
        if (!orderId) {
          ...
          // ✅ Fire Purchase only once after confirmed order
          const trackingId = String(res?.order?.tracking_id || Date.now());
          const alreadyTracked = sessionStorage.getItem(
            `purchase_tracked_${trackingId}`,
          );
          if (!alreadyTracked) {
            trackEvent(
              "Purchase",
              {
                value: Number(orderPayload.summary.total),
                currency: "PKR",
                content_type: "product",
                content_ids: orderPayload.items.map((i) => i.slug),
                contents: orderPayload.items.map((i) => ({
                  id: i.slug,
                  quantity: i.quantity,
                  item_price: Number(i.selectedVariant?.price || 0),
                })),
            ...
            sessionStorage.setItem(`purchase_tracked_${trackingId}`, "true");
          }
          ...
          logActivity({
            ...
            items: orderPayload.items,
            summary: orderPayload.summary,
            orderTrackingId: res?.order?.tracking_id ?? null,
          });
